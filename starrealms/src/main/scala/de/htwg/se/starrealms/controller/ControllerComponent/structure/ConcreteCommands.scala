...
case class BuyCardCommand(mediator: GameMediator, card: CardInterface) extends  CommandInterface {
...
case class DrawCardCommand(mediator: GameMediator) extends  CommandInterface {
...
case class StartTurnCommand(mediator: GameMediator, countCards: Int) extends  CommandInterface {
...
case class EndTurnCommand(mediator: GameMediator) extends  CommandInterface {
  override def doMove: Unit = mediator.getGameLogic.endTurn; override def undoMove: Unit = mediator.getGameLogic.undoEndTurn }

case class InvalidCommand(input: String) extends  CommandInterface {
  override def doMove: Unit = println(s"Invalid command: $input"); override def undoMove: Unit = {} }

case class PlayCardCommand(mediator: GameMediator, card: CardInterface) extends  CommandInterface {
...
case class ReplenishTradeRowCommand(mediator: GameMediator) extends  CommandInterface {
...
case class ResetGameCommand(mediator: GameMediator) extends  CommandInterface {
